...
class MyClient(discord.Client):
    ...
    async def daily_task(self):
        ...
        try:
            ...
            print("Unable to query daily_tasks")
            cursor.close()
            return
        ...
        if result:
            ...
            if last_authorized:
                ...
                if role_data:
                    ...
                    print(f"Possible database error, exiting")
                    os._exit(1)
        else:
            # user has no token
            # redirect to auth
            ...
            #await self.log_message(member.guild.id,f"Sent link to user {member.name} with user_id {member.id} at {datetime.datetime.now()}.")
            except mysql.connector.Error as err:
                print(f"Database Error: {err}")
                print(f"Database write error on_member_join exiting")
                os._exit(1)
